...
class System:
  beams: List[Tuple[Beam, Vector3, float]] = [] # the tuple vector is the beam's position, with respect to the center of the
  ...
  def solveSystem(self):
    ...
    h: List[Tuple[Vector3, Vector3]] = [] # the first vector is the reaction force from the support and the second
    v: List[Tuple[Vector3, Vector3]] = [] # one is its position with respect to the center of the coordinate system
    m: List[Vector3] = [] # the vector is the reaction force from the support
    ...
    for beam in self.beams:
      if isinstance(beam[0].start, Support):
        ...
        pos: Vector3 = beam[1]
        v.append([reaction, pos])
        if reaction.x != None:
          h.append([reaction, pos])
        if reaction.z != None:
          m.append([reaction, pos])
      ...
      for moment in beam[0].momentList:
        b.z -= moment.magnitude
